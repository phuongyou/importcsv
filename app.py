...
def process_excel_file(file_path: str, dry_run: bool = False, sheets: list = None) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            inserted = seed_business_units(cur)
            if not dry_run:
                conn.commit()
            results["steps"]["business_units"] = {
                "status": "completed",
                "message": f"Completed business_units - {inserted} inserted"
            }
            log.info(f"✓ business_units completed - {inserted} inserted")
        except Exception as e:
        ...
        try:
            ...
            client_map, inserted, skipped = import_clients(cur)
            if not dry_run:
                conn.commit()
            results["steps"]["clients"] = {
                "status": "completed",
                "message": f"Completed clients - {inserted} inserted, {skipped} skipped"
            }
            log.info(f"✓ clients completed - {inserted} inserted, {skipped} skipped")
        except Exception as e:
        ...
        try:
            ...
            agr_map, inserted, skipped = import_agreements(cur, client_map)
            if not dry_run:
                conn.commit()
            results["steps"]["agreements"] = {
                "status": "completed",
                "message": f"Completed agreements - {inserted} inserted, {skipped} skipped"
            }
            log.info(f"✓ agreements completed - {inserted} inserted, {skipped} skipped")
        except Exception as e:
        ...
        try:
            ...
            results["steps"]["sows"] = {
                "status": "completed",
                "message": f"Completed sows - {count[0]} inserted, {count[1]} skipped"
            }
            log.info(f"✓ sows completed - {count[0]} inserted, {count[1]} skipped")
        except Exception as e:
        ...
        try:
            ...
            inserted, skipped = import_partnerships(cur)
            if not dry_run:
                conn.commit()
            results["steps"]["partnerships"] = {
                "status": "completed",
                "message": f"Completed partnerships - {inserted} inserted, {skipped} skipped"
            }
            log.info(f"✓ partnerships completed - {inserted} inserted, {skipped} skipped")
        except Exception as e:
        ...
        for step_name, step_info in results["steps"].items():
            if step_info["status"] == "completed":
                # Extract inserted count from message
                message = step_info.get("message", "")
                # Expected format: "Completed {step_name} - {count} inserted" or "X inserted, Y skipped" or "{count} records"
                import re
                # Try to match "X inserted" first
                match = re.search(r'(\d+)\s+inserted', message)
                if match:
                    count = int(match.group(1))
                    summary[step_name] = count
                    total_records += count
                else:
                    # Fallback to "X records" for backward compatibility
